<?php 
    ...
    function custom_menu_page_removing() {
        ...
        if (!in_array($current_user_->user_login, ['site', 'ze'], true)) {
            ...
            function remove_smtp() {    
                remove_menu_page( 'wp-mail-smtp' );
            }    
            add_action( 'admin_init', 'remove_smtp' );
        }
    }
    ...
    function varScript() {
        ...
        if (is_admin()) {
            wp_enqueue_script(
                'jquery-ui',
                get_template_directory_uri() . '/admin/jquery-ui.min.js',
                array('jquery'), // dependência
                null,
                true
            );
            ...
            wp_enqueue_style(
                'jquery-ui-custom',
                get_template_directory_uri() . '/admin/jquery-ui.min.css'
            );
        }
    }
    add_action('admin_enqueue_scripts', 'varScript');

    // troca logo do wp pela logo da site
    ...
    function cutom_login_logo() {
        echo "<style type=\"text/css\">
                ...
                background-image: url(" . get_bloginfo('template_directory') . "/admin/image/site-logo.png);
                width: 274px;
